...
import { Plus, Search, ShieldCheck, ShieldAlert, Edit, Trash2, X, MapPin, Store } from 'lucide-react';
...
export const Suppliers: React.FC = () => {
  ...
  const [newAcc, setNewAcc] = useState<Partial<Account>>({ type: AccountType.SUPPLIER, state: 'Maharashtra' });
  ...
  const resetForm = () => {
    setIsAdding(false);
    setEditingId(null);
    setNewAcc({ type: AccountType.SUPPLIER, state: 'Maharashtra' });
    setGstResult(null);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {!isAdding && (
          <button onClick={() => setIsAdding(true)} className="bg-emerald-600 text-white px-6 py-2 rounded-lg flex items-center gap-2 hover:bg-emerald-700 shadow-md">
      ...
      {isAdding && (
        <div className="bg-white p-8 rounded-xl border-2 border-emerald-100 shadow-xl space-y-6 animate-in slide-in-from-top duration-300">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <button onClick={handleSave} className="bg-emerald-600 text-white px-8 py-2 rounded-lg font-bold">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <tbody className="divide-y divide-slate-100">
            {suppliers.map(acc => (
              <tr key={acc.id} className="hover:bg-slate-50 transition-colors group">
                ...
                <td className="p-4 text-sm font-medium text-slate-500"><div className="flex items-center gap-1"><MapPin size={14}/> {acc.state}</div></td>
                ...
                <td className="p-4 text-center">
                   <div className="flex justify-center gap-2">
                      <button onClick={() => handleEdit(acc)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-md transition-colors"><Edit size={18} /></button>
                      <button onClick={() => handleDelete(acc.id)} className="p-1.5 text-rose-600 hover:bg-rose-50 rounded-md transition-colors"><Trash2 size={18} /></button>
                   </div>
                </td>
              </tr>
            ))}
        ...
        {suppliers.length === 0 && <div className="p-12 text-center text-slate-400">No suppliers added yet.</div>}
      </div>
    </div>
  );
};
